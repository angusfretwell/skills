...
2. Run `/mattpocock-skills:code-review` on the branch — the changes since its merge-base with the default branch. Brief each axis sub-agent to **return** its report, never to message it. Review the code only: do NOT run tests, lint, typecheck, or builds — CI owns those.
