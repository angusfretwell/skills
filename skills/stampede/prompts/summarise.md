...
1. If it names a PR, upsert ONE comment on it headed `## AFK summary` — find it by that heading; create it if absent, else edit it in place:
   ...
   **Outcome:** <outcome, or —>
   then one line per review round, QA round, and CI attempt, each saying only that it happened and how it ended (findings → addressed, clean, red → fixed). Counts only — the findings are on disk.
