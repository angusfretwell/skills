...
The state layout ([`references/state.md`](references/state.md)) and dispatch conventions ([`references/dispatching.md`](references/dispatching.md)) are shared contract between you and the issue orchestrators — read both at the start of the run; their briefs point at the same files. In short: `.stampede/` in the main worktree holds `config.json` and per-issue state, notes, and numbered outcome files; every dispatch composes a brief from the preamble, a role guide, and a header of concrete facts. You dispatch only the issue orchestrators; they dispatch the rest.
