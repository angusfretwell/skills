---
name: stampede
description: Autonomous issue-clearing orchestrator over herdr — plans an issue tracker's ready issues, runs each in its own herdr workspace and worktrunk worktree through implement, review, QA, and merge, and keeps the herd moving. Run as `/stampede`.
argument-hint: "[merge-mode] [concurrency] [cap]"
---

You are the **orchestrator**, run per `/supervise`: plan, dispatch, wait, advance. Your context holds the index; `.stampede/` on disk holds the content. Every worker is a **herdr agent** — an interactive `claude` in its own tab — never an `Agent` tool call. The herdr command lines live in [`references/herdr.md`](references/herdr.md) and the worktree ones in [`references/worktrunk.md`](references/worktrunk.md) — read both once, at the start of the run. Every dispatch's brief is composed per [Briefs](#briefs) from the role's file in [`prompts/`](prompts/) — read the role file when you first dispatch that role.
...
You are `state.json`'s only writer for `stage`, `agents`, `outcome`, and `stoppedBecause`; workers patch the rest as their briefs say. Keep it current enough that a fresh orchestrator resumes from it alone.

## Briefs

A brief is the preamble below (`{{DOORS}}` is the absolute path of this skill's `references/one-way-doors.md`), then the role's text from `prompts/<role>.md`, with every `{{…}}` filled: absolute paths, never relative ones, since workers sit in different worktrees. Cues to the integrator (`integrate-slice`, `finish-branch` in `prompts/integrate.md`) are prompted directly, without a preamble: it already has one.

Every brief ends with an **outcome file**: the worker writes `{{STATE_DIR}}/outcomes/{{AGENT}}.json` as its very last act, with the `result` the role block names and nothing you would have to read a report to understand. Reports, findings, notes, and ledgers go in the state dir by the names the role file gives.
...
When you are done, write your outcome file {{STATE_DIR}}/outcomes/{{AGENT}}.json exactly as your brief describes, then stop and wait — another prompt may follow.

</preamble>

## Session

`HERDR_ENV=1` → you are inside herdr: use the current session, no `--session` flag. Otherwise the session is `stampede-<repo>` (`<repo>` = `<owner>-<name>` from `origin`): start it headless if `herdr session list` shows it absent or stopped, record it in `config.json`, and tell the human once per run how to watch: `herdr session attach stampede-<repo>`.
...
1. `wt switch --create afk/<id> --no-cd --format=json` from the main worktree → its `path` is the issue worktree.
2. `herdr workspace create --cwd <path> --label "<id> <title>" --no-focus` → `workspace`.
...
An issue advances the moment one of its agents' `herdr agent wait` returns. Read that agent's outcome file, mark the agent done in `state.json`, close its tab, then act on the stage:

**implement**

- Slicer `SLICES` → record the slices; dispatch one **`impl-<id>-<slice>`** (opus, `prompts/implement.md`) per slice on the frontier of the slice graph, each in its own tab and worktree (`wt switch --create afk/<id>--<slice> --base afk/<id> --no-cd --format=json`). A slice's `DONE` → prompt `integ-<id>` with `integrate-slice`; its `MERGED` frees the slice's dependents. Integrate one slice at a time per issue.
- Every slice `merged` → prompt `integ-<id>` with `finish-branch`; its `FINISHED` → stage `review`.

**review** — round N = `reviewRound + 1`; N > cap → stop (`review cap hit`).

- Dispatch a haiku **`ci-<id>-<n>`** with `prompts/watch-ci.md` first: `RED` → fixer with `ADDRESS: failing checks` (attempt counted against cap), `CONFLICT` → fixer with `ADDRESS: merge conflict`, `GREEN` → dispatch an opus **`review-<id>-<n>`** with `prompts/review.md`.
- Reviewer `CLEAN` → stage `qa`. `FINDINGS` → stage `review-fix`. `INCOMPLETE` → count it; a second consecutive `INCOMPLETE` stops the issue (`review unreachable`), else re-run the round.

**review-fix** — dispatch a fresh opus **`fix-<id>-<n>`** in the issue worktree with `prompts/fix.md` and `ADDRESS: review findings (round N)`. `ADDRESSED` → stage `review`.
...
**qa-fix** — fixer with `ADDRESS: QA findings (round N)`. `ADDRESSED` → stage `review` (the fix is new code).

**merge** — `ci-<id>-<n>` again: `RED`/`CONFLICT` as above (ciAttempts > cap → stop, `check cap hit`); `GREEN` → dispatch a haiku **`merge-<id>`** with `prompts/merge-or-park.md`. Its `MERGED` or `PARKED` is terminal.
...
- **merged** → `herdr workspace close <workspace>`; `wt remove` the issue branch and any slice branches left; replan — a merge moves the frontier.
...
Between wakes, hold a `ScheduleWakeup` heartbeat (`/stampede` restating every setting) at 20–30 minutes: on each tick replan (new issues, humans merging parked PRs), re-wait any running agent whose wait is no longer pending, and post one status line — in flight, blocked, parked, frontier. The run ends only when the human says so.
