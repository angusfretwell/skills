...
- A project's `.config/wt.toml` hooks run on switch and remove. If `wt` stops for an approval prompt, the approval is the human's — surface it per Blocked and wait for their answer.
