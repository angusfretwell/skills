# herdr mechanics

The installed binary is the authority on syntax — `herdr <group>` with no subcommand prints a group's commands. This file holds the recipes the orchestrator repeats and the gotchas no `--help` confesses. Every herdr command returns JSON: read ids and states from `.result`, never predict them.

## Targeting the session

Inside herdr (`HERDR_ENV=1`) run `herdr …` bare. Outside, prefix every command with `--session stampede-<repo>`: `herdr --session stampede-<repo> workspace list`. The flag is not an environment variable — it goes on every call.

Starting a session headless: `herdr --session stampede-<repo> server` run in the background (Bash `run_in_background`) — it is the server process and holds the foreground. It is up when `herdr --session stampede-<repo> workspace list` returns JSON rather than `server_not_running`. `herdr session list` shows it `running`. Leave it running at the end of a run; the human attaches with `herdr session attach stampede-<repo>`. `server stop` and `session delete` are the human's to run.

## Dispatching a worker

Every worker is an interactive `claude` started in a fresh tab of the issue's workspace, named for its role, prompted with a brief on disk.

1. Write the brief to `.stampede/<id>/briefs/<name>.md` — composed per `SKILL.md`'s Briefs section.
2. `herdr tab create --workspace <w> --cwd <worktree> --label "<name>"` → `.result.root_pane.pane_id` and `.result.tab.tab_id`; record the tab under `agents.<name>` in `state.json`.
3. `herdr agent start <name> --kind claude --pane <pane_id> --timeout 120000 -- --dangerously-skip-permissions --model <model>` — returns once claude is idle. `agent_not_ready` means claude is stuck at a startup dialog: `agent read` it and treat as blocked.
4. `herdr agent prompt <name> "Read and follow the brief at <absolute brief path>. It names the outcome file to write when you are done."` — no `--wait` here; the wait is the next step.
5. `herdr agent wait <name>` in a background Bash call. It returns on `idle`, `done`, or `blocked`; read `.result.agent.agent_status`. `done`/`idle` with an outcome file on disk is a finished dispatch; without one, prompt the agent once to write it, then treat a second empty return as the agent missing.

The integrator is the one long-lived worker: start it once in the workspace's root pane (`.result.root_pane.pane_id` from `workspace create`), then each later cue is `agent prompt integ-<id> "…"` followed by a fresh background `agent wait`. Prompt it only when `herdr agent get integ-<id>` shows `idle` or `done`; `agent_blocked` on a prompt means it is asking something — read the pane.

Agent names: `[a-z][a-z0-9_-]{0,31}`, unique among live agents in the session. The role-id-round scheme in `SKILL.md` keeps them unique; a slice id that pushes `impl-<id>-<slice>` past 32 characters is truncated from the slice end.

## Reading a blocked pane

`herdr agent read <name> --source recent-unwrapped --lines 80` shows what it is asking. Claude runs on the alternate screen, so a finished response may not be recoverable this way — that is why outcomes live in files, not panes. Answer a question only by `agent prompt` (text) or `agent send-keys <name> <key>` (a dialog) and only after the human has decided; record the decision in `state.json`.

## Closing

- A finished worker's tab: `herdr tab close <tab_id>` — this ends the claude process inside it.
- A terminated issue: `herdr workspace close <workspace_id>` closes every tab, the integrator included.
- Close only what this run or a prior `state.json` created; everything else in the session is the human's.
