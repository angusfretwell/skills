...
Keep a single **interview** agent (opus) in a tab beside yours: spawn it there if absent with the brief `prompts/interview.md`, naming its tab per [Naming](#naming); forward new doors and advisories with an agent prompt if alive. Either way, send a push notification naming the issues and questions.
