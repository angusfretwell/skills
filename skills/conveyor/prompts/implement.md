...
Execute the plan — the whole issue, or the one slice named in your brief.

- **Unsliced:** work in the issue worktree on `conveyor/<id>`.
- **Slice:** create your own worktree and branch `conveyor/<id>--<slice>` off the issue branch with `wt`; stay strictly inside your slice's scope from `plan.md` — another agent owns the neighboring slices, and scope creep collides with them.
