...
2. **Scaffold.** Create the issue worktree and branch per the `/worktrunk:worktrunk` skill, push it, and open a **draft PR** per the `/open-pr` skill, except the body is a placeholder: one line pointing at the issue — the real description comes after implementation.
