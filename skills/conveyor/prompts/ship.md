...
- **Mode `auto`:** mark the draft PR ready for review, confirm CI is green and the PR mergeable (either false: outcome verdict `not-ready`, `summary` saying which), then merge the PR and move the issue to done in the tracker. Outcome verdict: `done`.
